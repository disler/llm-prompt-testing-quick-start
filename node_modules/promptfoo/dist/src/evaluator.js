...
exports.evaluate = exports.renderPrompt = exports.DEFAULT_MAX_CONCURRENCY = void 0;
...
function generateVarCombinations(vars) {
    ...
    for (const key of keys) {
        let values = Array.isArray(vars[key]) ? vars[key] : [vars[key]];
...
async function renderPrompt(prompt, vars, nunjucksFilters) {
...
class Evaluator {
    ...
    async runEval({ provider, prompt, test, delay, nunjucksFilters: filters, }) {
        ...
        try {
            ...
            if (response.error) {
                ...
                if (transform) {
                    processedResponse.output = (0, util_1.transformOutput)(transform, processedResponse.output, { vars });
                }
    ...
    async evaluate() {
        ...
        const results = [];
        let numComplete = 0;
        await async_1.default.forEachOfLimit(runEvalOptions, concurrency, async (evalStep, index) => {
